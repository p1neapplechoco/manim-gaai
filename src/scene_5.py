...
class AttentionMechanism(Scene):
    def construct(self):
        ...
        for i in range(n_nodes):
            for j in range(i + 1, n_nodes):
                edge = Line(
                    ...
                    color=GOLD, stroke_width=0.6, stroke_opacity=0.25,
                )
        ...
        self.play(
            FadeIn(intro_nodes), FadeIn(intro_edges),
            run_time=0.7,
        )
        self.play(
            Write(trans_label),
        ...
        word_mobs = VGroup(
            *[Text(w, font_size=22, color=WHITE) for w in words]
        )
        # Arrange in two rows for readability
        row1 = VGroup(*word_mobs[:6]).arrange(RIGHT, buff=0.22)
        row2 = VGroup(*word_mobs[6:]).arrange(RIGHT, buff=0.22)
        sentence = VGroup(row1, row2).arrange(DOWN, buff=0.4, aligned_edge=LEFT)
        sentence.move_to(UP * 0.5)

        self.play(
            LaggedStart(
                *[FadeIn(w, shift=UP * 0.1) for w in word_mobs],
                lag_ratio=0.05,
            ),
            run_time=1.0,
        )
        ...
        it_highlight = SurroundingRectangle(
            it_word, color=GOLD, buff=0.08,
            corner_radius=0.06, stroke_width=2.5,
            fill_color=GOLD, fill_opacity=0.1,
        )
        ...
        # Highlight the two candidates
        animal_word = word_mobs[1]  # "animal"
        street_word = word_mobs[5]  # "street"

        animal_hl = SurroundingRectangle(
            animal_word, color=GREEN, buff=0.08,
            corner_radius=0.06, stroke_width=2,
            fill_color=GREEN, fill_opacity=0.08,
        )
        street_hl = SurroundingRectangle(
            street_word, color=RED, buff=0.08,
            corner_radius=0.06, stroke_width=2,
            fill_color=RED, fill_opacity=0.08,
        )

        self.play(Create(animal_hl), Create(street_hl), run_time=0.5)

        # Question arrows from "it" to both candidates
        q_arrow_animal = CurvedArrow(
            it_word.get_top() + UP * 0.15,
            animal_word.get_top() + UP * 0.15,
            angle=-0.5,
            color=GREEN, stroke_width=1.8, tip_length=0.12,
        )
        q_arrow_street = CurvedArrow(
            it_word.get_top() + UP * 0.15,
            street_word.get_top() + UP * 0.15,
            angle=-0.4,
            color=RED, stroke_width=1.8, tip_length=0.12,
        )

        q_label = Text('"it" = ?', font_size=20, color=GOLD)
        q_label.next_to(sentence, DOWN, buff=0.8)

        animal_opt = Text("animal?", font_size=16, color=GREEN)
        street_opt = Text("street?", font_size=16, color=RED)
        animal_opt.next_to(q_label, LEFT, buff=0.8)
        street_opt.next_to(q_label, RIGHT, buff=0.8)

        self.play(
            Create(q_arrow_animal), Create(q_arrow_street),
            FadeIn(q_label),
        ...
        chain_words = ["The", "animal", "didn't", "cross", "the", "street", "because", "it"]
        ...
        for i in range(n_chain):
            ...
            cell = RoundedRectangle(
                width=c_w, height=0.4, corner_radius=0.06,
            ...
            if i < n_chain - 1:
                arr = Arrow(
                    cell.get_right(), cell.get_right() + RIGHT * 0.25,
                    buff=0.04, color=GOLD, stroke_width=1.2,
        ...
        self.play(
            FadeIn(chain_cells), FadeIn(chain_labels),
        ...
        for i in range(1, 7):
            seg = Line(
                ...
                color=GREEN, stroke_width=2.5, stroke_opacity=max(1.0 - i * 0.15, 0.1),
            )
            hop_path.add(seg)
        ...
        tf_title = Text("Transformer", font_size=20, color=ACCENT)
        tf_title.to_edge(UP, buff=0.4)
        self.play(Write(tf_title), run_time=0.4)

        # Sentence tokens as dots in a line
        attn_words = ["The", "animal", "didn't", "cross", "the", "street", "because", "it"]
        ...
        for i in range(n_attn):
            ...
            dot.move_to(RIGHT * cx + UP * 0.3)
            attn_dots.add(dot)

            lb = Text(attn_words[i], font_size=12, color=WHITE)
        ...
        self.play(
            LaggedStart(
                *[FadeIn(d, scale=0.5) for d in attn_dots], lag_ratio=0.04,
            ),
            LaggedStart(
                *[FadeIn(l) for l in attn_labels], lag_ratio=0.04,
            ),
            run_time=0.7,
        )
        ...
        attn_line_main = Line(
            attn_dots[7].get_center(),
            attn_dots[1].get_center(),
            color=GOLD, stroke_width=3.5, stroke_opacity=0.9,
        )

        # Dimmer attention lines from "it" to other tokens
        ...
        for i in range(n_attn):
            ...
            line = Line(
                attn_dots[7].get_center(),
                attn_dots[i].get_center(),
                color=ACCENT, stroke_width=0.8,
                stroke_opacity=0.15,
            )
            attn_lines_dim.add(line)

        self.play(
            Create(attn_line_main),
            run_time=0.6,
        )
        self.play(
            Flash(attn_dots[1].get_center(), color=GREEN, flash_radius=0.3, num_lines=6),
            run_time=0.4,
        )
        self.play(
        ...
        weight_main.move_to(
            (attn_dots[7].get_center() + attn_dots[1].get_center()) / 2 + UP * 0.35
        )
        self.play(FadeIn(weight_main, scale=1.1), run_time=0.3)

        direct_label = Text("direct connection — no chain needed", font_size=16, color=GOLD)
        ...
        for i in range(n_full):
            for j in range(i + 1, n_full):
                e = Line(
                    full_dots[i].get_center(), full_dots[j].get_center(),
                    color=GOLD, stroke_width=1.0, stroke_opacity=0.3,
                )
                full_edges.add(e)

        self.play(
            FadeIn(full_dots), FadeIn(full_labels),
        ...
        bag = RoundedRectangle(
            width=1.4, height=1.8, corner_radius=0.25,
            color=GOLD, stroke_width=2,
            fill_color="#1A1500", fill_opacity=1,
        )
        ...
        candies = VGroup()
        candy_positions = [
            UP * 0.3 + LEFT * 0.15, UP * 0.3 + RIGHT * 0.2,
            UP * 0.0 + LEFT * 0.1, UP * 0.0 + RIGHT * 0.25,
            DOWN * 0.3, DOWN * 0.3 + RIGHT * 0.15,
        ]
        ...
        for i in range(3):
            person = Circle(
                radius=0.2, color=WHITE, stroke_width=1.5,
        ...
        for p in few_people:
            arr = Arrow(
                bag.get_right(), p.get_left(),
                buff=0.1, color=GOLD, stroke_width=2.5,
                max_tip_length_to_length_ratio=0.2,
            )
            few_arrows.add(arr)

        self.play(
            FadeIn(few_people), FadeIn(few_label),
        ...
        self.play(
            FadeOut(few_people), FadeOut(few_label),
            FadeOut(few_arrows), FadeOut(enough),
            run_time=0.4,
        )
        ...
        for i in range(10):
            person = Circle(
                radius=0.13, color=WHITE, stroke_width=1,
                fill_color="#111111", fill_opacity=1,
            )
            row = i // 2
            col = i % 2
            person.move_to(
                RIGHT * (0.8 + col * 0.5) + UP * (1.5 - row * 0.55)
            )
        ...
        for p in many_people:
            arr = Arrow(
                bag.get_right(), p.get_left(),
                buff=0.06, color=GOLD, stroke_width=0.6,
                stroke_opacity=0.4,
                max_tip_length_to_length_ratio=0.3,
            )
            many_arrows.add(arr)
        ...
        self.play(
            FadeIn(many_people), FadeIn(many_label),
        ...
        imp_glow = important.copy().set_stroke(GREEN, width=3).set_fill(GREEN, opacity=0.2)
        ...
        for r in range(short_n):
            for c in range(short_n):
                ...
                if r == 3 and c == 0:
                    opacity = 0.7
                cell = Square(
                    side_length=0.4, color=GOLD, stroke_width=0.5,
                    fill_color=GOLD, fill_opacity=opacity,
                )
                cell.move_to(
                    LEFT * 3.2 + RIGHT * c * 0.42 + DOWN * r * 0.42
                    + UP * 1.3 + LEFT * 0.6
                )
                short_grid.add(cell)
        ...
        for r in range(long_n):
            for c in range(long_n):
                ...
                if r == c:
                    opacity = 0.35
                cell = Square(
                    side_length=0.25, color=GOLD, stroke_width=0.3,
                    fill_color=GOLD, fill_opacity=opacity,
                )
                cell.move_to(
                    RIGHT * 2.8 + RIGHT * c * 0.27 + DOWN * r * 0.27
                    + UP * 1.8 + LEFT * 0.9
                )
                long_grid.add(cell)
        ...
        self.play(
            FadeIn(header_mobs), Create(h_line),
            run_time=0.4,
        )
        ...
        worst_box = SurroundingRectangle(
            ...
            color=RED, buff=0.1, corner_radius=0.06,
            stroke_width=1.5, fill_color=RED, fill_opacity=0.05,
        )
        ...
        for r in range(small_n):
            for c in range(small_n):
                cell = Square(
                    side_length=0.3, color=ACCENT, stroke_width=0.8,
                    fill_color=ACCENT, fill_opacity=0.3,
                )
                cell.move_to(
                    LEFT * 4.0 + RIGHT * c * 0.32 + DOWN * r * 0.32 + UP * 0.5
                )
        ...
        for r in range(large_n):
            for c in range(large_n):
                cell = Square(
                    side_length=0.2, color=RED, stroke_width=0.5,
                    fill_color=RED, fill_opacity=0.2,
                )
                cell.move_to(
                    LEFT * 2.0 + RIGHT * c * 0.22 + DOWN * r * 0.22 + UP * 0.7
                )
        ...
        self.play(
            ...
            FadeIn(small_n_label), FadeIn(small_ops),
            run_time=0.5,
        )
        self.play(
            LaggedStart(*[FadeIn(c) for c in large_cost_grid], lag_ratio=0.01),
            FadeIn(large_n_label), FadeIn(large_ops),
            run_time=0.5,
        )
        ...
        for i in range(35):
            ...
            col = i % 7
            dot = Dot(
                radius=0.06, color=DIM2, fill_opacity=0.3,
            )
            dot.move_to(
                RIGHT * 2.0 + RIGHT * col * 0.35 + DOWN * row * 0.3 + UP * 0.8
            )
            hay_tokens.add(dot)
        ...
        self.play(
            ...
            Flash(
                ...
                color=GOLD, flash_radius=0.2, num_lines=6,
            ),
            run_time=0.4,
        )
        ...
        sum_box = SurroundingRectangle(
            summary, color=SEPIA, buff=0.2,
            corner_radius=0.12, fill_color=SEPIA_DARK,
